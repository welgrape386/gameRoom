...
export function CenterArea({ chars, playerCharId }: CenterAreaProps) {
  return (
    ...
    >
      ...
      >
        {chars.map(char => (
          ...
          >
            ...
            <div style={{ position: 'absolute', bottom: -24, fontSize: 'clamp(9px, 0.7vw, 13px)' }}>
              {char.name}
            </div>
          </div>
        ))}
      ...
      {/* Ambient lights */}
      <div className="absolute top-0 left-1/4 w-72 h-72 rounded-full opacity-20 blur-3xl" style={{ background: '#a855f7' }} />
      <div className="absolute bottom-0 right-1/4 w-72 h-72 rounded-full opacity-20 blur-3xl" style={{ background: '#38bdf8' }} />
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse at center, transparent 35%, rgba(4,4,10,0.75) 100%)',
        }}
      />
    </div>
  );
